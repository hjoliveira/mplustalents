...
describe("MPlusTalents", function()
    ...
    describe("when entering a known dungeon", function()
        before_each(function()
            -- Simulate being inside Ara-Kara, City of Echoes (instanceID = 2773)
            _G._instanceName = "Ara-Kara, City of Echoes"
        ...
        it("includes the dungeon name in the output", function()
            ...
            assert.is_truthy(output:find("Ara%-Kara"))
        end)

        it("includes the class name in the output", function()
            addon.fireEvent("PLAYER_ENTERING_WORLD", true, false)
            local output = table.concat(addon.getPrinted(), "\n")
            assert.is_truthy(output:find("Warrior"))
        end)
    ...
    describe("when entering a non-dungeon instance", function()
        before_each(function()
